...
export class BlogService implements IBlogService {
  ...
  async findArchivedBlogs(authorId: Types.ObjectId, page: number): Promise<{ archivedBlogs: IBlogModel[], totalPages: number }> {
    const docsPerPage = 12
    const skip = (page - 1) * docsPerPage
    const { archivedBlogs, totalCount } = await this.blogRepository.findArchivedBlogs(authorId, skip, docsPerPage);

    const blogsWithVoteStatus: BlogWithVoteStatus[] = await Promise.all(
      archivedBlogs.map(async (blog: IBlogModel) => {
        const vote = await this.voteRepository.findVoteByUserAndBlog(authorId, new Types.ObjectId(blog._id as string));
        const voteStatus = await this.getVoteStatus(vote);
    ...
    return { archivedBlogs: blogsWithVoteStatus, totalPages: Math.ceil(totalCount / docsPerPage) }
  }
  ...
  async archiveBlog(
    ...
    const archivedBlog = await this.blogRepository.archiveBlog(
      blogId,
      authorId,
      action
    );
